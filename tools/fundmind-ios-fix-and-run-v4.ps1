...
function Run-Proc($file,[string[]]$argv){
  $argstr = if($argv){ $argv -join " " } else { "" }
  Log ("Run: " + $file + " " + $argstr)
...
# Move config files from app/ to root if any were misplaced
...
if(HasProp $pkg.dependencies "expo-dev-client" -or HasProp $pkg.devDependencies "expo-dev-client"){ Log "Detected expo-dev-client: Expo Go will not load native modules on iOS. Use a custom Dev Client." }
Log "Starting Expo dev server (interactive)..."
$expoCmd = "npx expo start --tunnel"
if($env:CLEAR -eq "1"){ $expoCmd = $expoCmd + " --clear"; Log "Metro cache clear enabled" }
Write-Host "----- Metro output below -----"
& cmd.exe /c $expoCmd
