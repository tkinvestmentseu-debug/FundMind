...
try{
  ...
  # Odczytaj SDK i dobierz babel/args
  ...
  # Upewnij main=expo-router/entry
  $pkgPath = Join-Path $projectRoot "package.json"
  if(Test-Path $pkgPath){ try{ $pkg=Get-Content $pkgPath -Raw | ConvertFrom-Json; if(-not ($pkg.PSObject.Properties.Name -contains "main") -or $pkg.main -ne "expo-router/entry"){ $pkg.main="expo-router/entry"; ($pkg|ConvertTo-Json -Depth 30) | Set-Content -Path $pkgPath -Encoding UTF8; Log "package.json: set main=expo-router/entry" } } catch { Save-AntiErr "AE-49-JSON" ("package.json invalid: " + $_.Exception.Message) } }

  # AE-51/48: PATH/COMSPEC + brak WSL
  ...
  # Start CLI bez npx (eliminuje notFoundError na spawn npx) — AE-47: BEZ sklejania z cudzysłowami
  ...
  try{
    ...
    Save-AntiErr "AE-48-SPAWN" ("spawn failed: " + $_.Exception.Message)
    throw
  }
} catch {
  Save-AntiErr "AE-47-FATAL" ($_.Exception.Message)
