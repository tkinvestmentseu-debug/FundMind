...
function Harden-Env(){
  ...
  # pe?ny debug, ?eby z?apa? "which command not found"
  $env:DEBUG="cross-spawn,expo:*,*spawn*,metro:*"
  $env:EXPO_DEBUG="1"
}
...
try{
  ...
  foreach($n in @("node","npm","git","cmd","powershell")){
    try{ $p=(Get-Command $n -ErrorAction SilentlyContinue).Source; ("FOUND {0}: {1}" -f $n,$p) | Out-File $startLog -Append } catch { ("MISS {0}" -f $n) | Out-File $startLog -Append }
  }
  ("PATH="+$env:PATH) | Out-File $startLog -Append
  ("DEBUG="+$env:DEBUG) | Out-File $startLog -Append
  "" | Out-File $startLog -Append

  # STRATEGIA 1: node cli.js (preferowana)
  if($cliJs){
    $cmd="node"; $args=@($cliJs,"start") + $argsArr
    AE "AE-59 SPAWN_TARGET" ("node "+($args -join " "))
    Log ("Starting (cli): node "+($args -join " "))
    & $cmd $args 2>&1 | Tee-Object -FilePath $startLog
    $code = $LASTEXITCODE
    if($code -ne 0){ AE "AE-71 START_EXIT_NONZERO" ("cli.js exit "+$code) }
  }

  # STRATEGIA 2: expo.cmd
  if(-not $cliJs -and (Test-Path $expoCmd)){
    AE "AE-52 CLI_NOT_FOUND" "fallback to .bin\expo.cmd"
    $args=@("start") + $argsArr
    Log ("Starting (.bin\expo): "+$expoCmd+" "+($args -join " "))
    & $expoCmd $args 2>&1 | Tee-Object -FilePath $startLog
    $code = $LASTEXITCODE
    if($code -ne 0){ AE "AE-74 EXPOCMD_SPAWN_ERR" ("expo.cmd exit "+$code) }
  }

  # STRATEGIA 3: npx expo
  if((-not $cliJs) -and (-not (Test-Path $expoCmd))){
    AE "AE-52 CLI_NOT_FOUND" "fallback to npx expo"
    $args=@("expo","start") + $argsArr
    Log ("Starting (npx): npx "+($args -join " "))
    & npx $args 2>&1 | Tee-Object -FilePath $startLog
    $code = $LASTEXITCODE
    if($code -ne 0){ AE "AE-71 START_EXIT_NONZERO" ("npx exit "+$code) }
  }

  # Post-analiza logu ? wykrycie ENOENT / "nie mo?e odnale?? ?cie?ki"
  if(Test-Path $startLog){
    $content = Get-Content $startLog -Raw
    if($content -match 'ENOENT|notFoundError|nie mo'){
      AE "AE-73 CROSSSPAWN_NOTFOUND" "spawn path error detected in expo start output"
    }
  } else {
    AE "AE-72 START_RETURNED_EARLY" "no start log generated"
  }

  Log ("Done. Log: "+$startLog)
}
catch{
