...
function Ensure-Shims(){ $shimDir = Join-Path $toolsDir "shims"; if(!(Test-Path $shimDir)){ New-Item -ItemType Directory -Force -Path $shimDir | Out-Null }
  $adb = Join-Path $shimDir "adb.cmd"; if(!(Test-Path $adb)){ @(":: AE-61 SHIM_ADB","@echo off","echo [shim] adb %*","exit /b 0") | Set-Content -Path $adb -Encoding ASCII }
  Append-Path @($shimDir) ; Log "Shim dir on PATH" }
...
try{
  ...
  # PATH/COMSPEC + rozszerzone systemowe katalogi
  ...
  # AE-60: Debug cross-spawn/Expo
  $env:DEBUG="cross-spawn,expo:*,*spawn*"; $env:EXPO_DEBUG="1"
  Ensure-ExpoInstalled $sdk
  Ensure-Shims
  if(Test-Path ".\.expo"){ Remove-Item -Recurse -Force ".\.expo" }
  if(Test-Path ".\.expo-shared"){ Remove-Item -Recurse -Force ".\.expo-shared" }
  # AE-62: autodetekcja CLI
  $cliCandidates = Get-ChildItem -Path (Join-Path $projectRoot "node_modules\expo") -Recurse -Filter "cli.*" -ErrorAction SilentlyContinue | Where-Object { $_.Name -match "^cli\.(c?js)$" } | Select-Object -First 1
  $cliJs = if($cliCandidates){ $cliCandidates.FullName } else { $null }
  ...
  if($cliJs -and (Test-Path $cliJs)){
    $cmdLine = 'node "{0}" start {1}' -f $cliJs, $startArgs
    Log ("Starting (cli): " + $cmdLine); Log ("Log file: " + $startLog)
    ...
    Log ("Starting (.bin\expo): " + $cmdLine); Log ("Log file: " + $startLog)
    Save-AE "AE-59 SPAWN_TARGET" $cmdLine
    & cmd.exe /c $cmdLine *>&1 | Tee-Object -FilePath $startLog
  } else {
    Save-AE "AE-52 CLI_NOT_FOUND" "fallback to npx expo"
    $cmdLine = 'npx expo start {0}' -f $startArgs
    Log ("Starting (npx): " + $cmdLine); Log ("Log file: " + $startLog)
    Save-AE "AE-59 SPAWN_TARGET" $cmdLine
    & cmd.exe /c $cmdLine *>&1 | Tee-Object -FilePath $startLog
  }
} catch {
