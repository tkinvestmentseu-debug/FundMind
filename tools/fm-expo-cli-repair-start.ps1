...
function Ensure-Expo-And-CLI([string]$sdk){
  ...
  $changed=$false
  if(-not ($pkg.dependencies.PSObject.Properties.Name -contains "expo") -or $pkg.dependencies.expo -notlike $want){ $pkg.dependencies.expo=$want; $changed=$true; Log ("Pinned expo "+$want+" (AE-55)") }
  # dopilnuj @expo/cli (fallback gdy expo/bin/cli.js brak)
  if(-not $pkg.devDependencies){ $pkg | Add-Member -Name devDependencies -MemberType NoteProperty -Value (@{}) }
  if(-not ($pkg.devDependencies.PSObject.Properties.Name -contains "@expo/cli")){ $pkg.devDependencies."@expo/cli"="^0.17.0"; $changed=$true; Log "Added @expo/cli devDep" }
  if($changed){ Save-Json $pkgP $pkg }
...
function Start-Expo([string[]]$argsArr){
  ...
  if(Test-Path $cli1){
    $all=@($cli1,"start") + $argsArr
    AE "AE-59 SPAWN_TARGET" ("node "+($all -join " "))
    Log "Starting (expo/bin/cli.js) LAN?"
    & node $all 2> $errLog | Tee-Object -FilePath $outLog
    return
  }
  if(Test-Path $cli2){
    $all=@($cli2,"start") + $argsArr
    AE "AE-59 SPAWN_TARGET" ("node "+($all -join " "))
    Log "Starting (@expo/cli) LAN?"
    & node $all 2> $errLog | Tee-Object -FilePath $outLog
    return
  }
...
try{
  ...
  # wymu? main=expo-router/entry (Expo Router)
  ...
  # wyczy?? stare cache Expo lokalnie
