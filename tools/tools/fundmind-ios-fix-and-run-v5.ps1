...
function Invoke-CmdLine([string]$cmd){ Log ("Run: " + $cmd); $psi = New-Object System.Diagnostics.ProcessStartInfo; $psi.FileName = "cmd.exe"; $psi.Arguments = "/d /s /c " + $cmd; $psi.WorkingDirectory = $projectRoot; $psi.UseShellExecute = $false; $psi.RedirectStandardOutput = $true; $psi.RedirectStandardError = $true; $psi.CreateNoWindow = $true; $p = [System.Diagnostics.Process]::Start($psi); $out = $p.StandardOutput.ReadToEnd(); $err = $p.StandardError.ReadToEnd(); $p.WaitForExit(); if($out){ Add-Content -Path $log -Value $out -Encoding UTF8 } if($err){ Add-Content -Path $log -Value $err -Encoding UTF8 } if($p.ExitCode -ne 0){ throw ("Command failed (" + $cmd + ") exit " + $p.ExitCode) } }
Log "Start v5"
...
if(-not (Test-Path $babelPath)){ $babel = @("module.exports = function(api) {","  api.cache(true);","  return {","    presets: ['babel-preset-expo'],","    plugins: ['expo-router/babel']","  };","};"); Set-Content -Path $babelPath -Value $babel -Encoding UTF8; Log "babel.config.js created" } else { $txt = Get-Content -Raw -Path $babelPath; if(($txt -notmatch "expo-router/babel") -or ($txt -notmatch "babel-preset-expo")){ Copy-Item $babelPath ($babelPath + ".bak." + $ts) -Force; $babel = @("module.exports = function(api) {","  api.cache(true);","  return {","    presets: ['babel-preset-expo'],","    plugins: ['expo-router/babel']","  };","};"); Set-Content -Path $babelPath -Value $babel -Encoding UTF8; Log "babel.config.js replaced" } else { Log "babel.config.js OK" } }
# app.json ensure
...
Invoke-CmdLine "npx expo install expo react react-native expo-router"
Log "Starting Expo dev server (tunnel)..."
$extra = ""
if($env:CLEAR -eq "1"){ $extra = " --clear"; Log "Metro cache clear enabled" }
& npx expo start --tunnel $extra
