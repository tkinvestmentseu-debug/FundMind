...
export default function RulesScreen() {
  ...
  const addRule = () => {
    ...
    if (!isFinite(val) || val <= 0) {
      Alert.alert("Błąd", "Podaj poprawną kwotę progu.");
      return;
    }
    const r: Rule = {
      id: String(Date.now()),
  ...
  return (
    <View style={styles.page}>
      ...
      <ScrollView variant="screen" contentContainerStyle={{ padding: 16 }}>
        <View style={styles.card}>
          ...
          <Pressable onPress={addRule} style={styles.primary}>
            ...
            <Text style={styles.primaryText}>Dodaj regułę</Text>
          </Pressable>
          <Text style={styles.note}>
            *Integracja z powiadomieniami/analizą transakcji — do podpięcia w następnym kroku.
          </Text>
        </View>

        <Text style={styles.section}>Aktywne reguły</Text>
        {rules.length === 0 ? (
          <Text style={styles.empty}>Brak zdefiniowanych reguł.</Text>
        ) : (
          rules.map((r) => (
            <View key={r.id} style={styles.ruleItem}>
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.ruleLine}>
                  Dzienne wydatki &gt; {r.amount.toFixed(2)} PLN{" "}
                  {r.category ? `(${r.category})` : ""}
                </Text>
                <Text style={styles.ruleSub}>{r.active ? "Aktywna" : "Wstrzymana"}</Text>
              </View>
              <Pressable onPress={() => toggleActive(r.id)} style={styles.chip}>
                <Text style={styles.chipText}>{r.active ? "Wstrzymaj" : "Wznów"}</Text>
              </Pressable>
              <Pressable onPress={() => removeRule(r.id)} style={[styles.chip, { marginLeft: 8 }]}>
                <Text style={styles.chipText}>Usuń</Text>
              </Pressable>
            </View>
          ))
        )}
