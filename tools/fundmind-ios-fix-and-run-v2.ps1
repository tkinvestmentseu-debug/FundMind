...
function Run($exe,[string[]]$args){ Log ("Run: " + $exe + " " + ($args -join " ")); & $exe @args *>&1 | Tee-Object -FilePath $log -Append; if($LASTEXITCODE -ne 0){ throw ($exe + " failed with exit code " + $LASTEXITCODE) } }
Log "Start fix"
...
Run "npm" @("install")
Run "npx" @("expo","install","expo","react","react-native","expo-router")
