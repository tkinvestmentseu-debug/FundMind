...
if (Test-Path $tabsLayout) {
  ...
  # usuwamy duplikaty kalendarza
  $cleaned = $content -replace "(?ms)^\s*<Tabs\.Screen name='calendar'.*?\r?\n",""
  if ($cleaned -notmatch "name='calendar'") {
    $cleaned = $cleaned -replace '(<Tabs>)', "`$1`r`n      <Tabs.Screen name='calendar' options={{ title: 'Kalendarz' }} />"
  }
  $cleaned | Set-Content -Encoding UTF8 $tabsLayout
  Write-Host "[FIX] Updated (tabs)/_layout.tsx (single calendar tab)"
}
