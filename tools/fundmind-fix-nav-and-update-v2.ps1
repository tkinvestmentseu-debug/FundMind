# fundmind-fix-nav-and-update-v2.ps1 (Anti-Errors 36..43)
...
function Log-Info($m){ $t=Get-Date -Format s; Write-Host "[FixNAVv2] $m"; "$t [Info] $m" | Out-File "$logsDir\update.log" -Encoding UTF8 -Append }
function Log-Error($m){ $t=Get-Date -Format s; Write-Host "[FixNAVv2-Error] $m"; "$t [AntiError] $m" | Out-File $errorsLog -Encoding UTF8 -Append }
...
try {
  ...
  Log-Info "Start fix nav deps (no overrides) + EAS update"

  # 1) Ensure configs exist (babel/metro)
  ...
  if ($app.expo.PSObject.Properties.Name -contains "extra"){
    if ($app.expo.extra.PSObject.Properties.Name -contains "eas"){
      # Usuń tylko projectId albo całe 'eas'
      if ($app.expo.extra.eas.PSObject.Properties.Name -contains "projectId"){
        $app.expo.extra.eas.PSObject.Properties.Remove("projectId")
      }
  ...
  # 4) Pin package.json (SDK50 + router entry + nav 6.x) and REMOVE overrides
  ...
  # Move test renderer to dev only
  ...
  # REMOVE overrides to avoid EOVERRIDE (Anti-Error #42)
  if ($pkg.PSObject.Properties.Name -contains "overrides"){
    $pkg.PSObject.Properties.Remove("overrides")
    Log-Info "Removed npm overrides (prevent EOVERRIDE)"
  }

  ($pkg | ConvertTo-Json -Depth 20) | Set-Content -Path $pkgPath -Encoding UTF8
  Remove-BOM $pkgPath
  Log-Info "package.json pinned (no overrides)"

  # 5) Clean install (single source of truth = package.json)
  ...
  Log-Info "npm install clean (legacy-peer-deps)"
  npm install --legacy-peer-deps
  if ($LASTEXITCODE -ne 0){ Log-Error "npm install failed"; exit 1 }

  # 6) (Opcjonalnie) align natywnych modułów — pomijamy jeżeli już są w package.json (Anti-Error #43)
  # Log-Info "Native deps already pinned; skipping expo install"
  ...
  if (!(Test-Path "$projectRoot\.eas")){
    Log-Info "Running eas init (no --id)"
  ...
  # 8) Publish via EAS Update
  Log-Info "Publishing with eas update (branch main)"
  npx eas-cli update --branch main --message "FundMind nav 6.x, no overrides, JSON sanitized"
